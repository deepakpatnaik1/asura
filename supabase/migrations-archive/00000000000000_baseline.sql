...
-- ============================================================================
-- TABLE: superjournal
-- Full, uncompressed conversation turns (working memory)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_journal_embedding ON public.journal USING hnsw (embedding vector_cosine_ops);
CREATE INDEX IF NOT EXISTS journal_instruction_scope_idx ON public.journal(is_instruction, instruction_scope) WHERE is_instruction = true;
...
CREATE INDEX IF NOT EXISTS idx_user_roles_is_admin ON public.user_roles(is_admin) WHERE is_admin = true;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_active_reader_article') THEN
...
CREATE INDEX IF NOT EXISTS idx_article_charts_article_id ON public.article_charts(article_id);
CREATE INDEX IF NOT EXISTS idx_article_charts_user_id ON public.article_charts(user_id);
CREATE INDEX IF NOT EXISTS idx_article_charts_alt_text ON public.article_charts USING gin(to_tsvector('english', alt_text));
...
CREATE INDEX IF NOT EXISTS idx_article_chat_article_id ON public.article_chat(article_id);
CREATE INDEX IF NOT EXISTS idx_article_chat_user_id ON public.article_chat(user_id);
CREATE INDEX IF NOT EXISTS idx_article_chat_created_at ON public.article_chat(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_file_chunks_embedding ON public.file_chunks USING hnsw (embedding vector_cosine_ops);
...
-- ============================================================================
-- RLS POLICIES (all wrapped in conditional blocks for idempotency)
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own superjournal entries' AND tablename = 'superjournal') THEN
        CREATE POLICY "Users can view their own superjournal entries" ON public.superjournal FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert their own superjournal entries' AND tablename = 'superjournal') THEN
        CREATE POLICY "Users can insert their own superjournal entries" ON public.superjournal FOR INSERT WITH CHECK (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update their own superjournal entries' AND tablename = 'superjournal') THEN
        CREATE POLICY "Users can update their own superjournal entries" ON public.superjournal FOR UPDATE USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete their own superjournal entries' AND tablename = 'superjournal') THEN
        CREATE POLICY "Users can delete their own superjournal entries" ON public.superjournal FOR DELETE USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;

    -- journal policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own journal entries' AND tablename = 'journal') THEN
        CREATE POLICY "Users can view own journal entries" ON public.journal FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own journal entries' AND tablename = 'journal') THEN
        CREATE POLICY "Users can insert own journal entries" ON public.journal FOR INSERT WITH CHECK (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own journal entries' AND tablename = 'journal') THEN
        CREATE POLICY "Users can update own journal entries" ON public.journal FOR UPDATE USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own journal entries' AND tablename = 'journal') THEN
        CREATE POLICY "Users can delete own journal entries" ON public.journal FOR DELETE USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;

    -- models policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated users can view models' AND tablename = 'models') THEN
        CREATE POLICY "Authenticated users can view models" ON public.models FOR SELECT USING (auth.role() = 'authenticated');
    END IF;

    -- model_parameters policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated users can view model parameters' AND tablename = 'model_parameters') THEN
        CREATE POLICY "Authenticated users can view model parameters" ON public.model_parameters FOR SELECT USING (auth.role() = 'authenticated');
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own settings' AND tablename = 'user_settings') THEN
        CREATE POLICY "Users can view own settings" ON public.user_settings FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own settings' AND tablename = 'user_settings') THEN
        CREATE POLICY "Users can insert own settings" ON public.user_settings FOR INSERT WITH CHECK (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own settings' AND tablename = 'user_settings') THEN
        CREATE POLICY "Users can update own settings" ON public.user_settings FOR UPDATE USING (auth.uid() = user_id OR is_admin(auth.uid())) WITH CHECK (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own settings' AND tablename = 'user_settings') THEN
        CREATE POLICY "Users can delete own settings" ON public.user_settings FOR DELETE USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    ...
    -- token_usage policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own token usage' AND tablename = 'token_usage') THEN
        CREATE POLICY "Users can view own token usage" ON public.token_usage FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own token usage' AND tablename = 'token_usage') THEN
        CREATE POLICY "Users can insert own token usage" ON public.token_usage FOR INSERT WITH CHECK (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;

    -- articles policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own articles' AND tablename = 'articles') THEN
        CREATE POLICY "Users can view own articles" ON public.articles FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own articles' AND tablename = 'articles') THEN
        CREATE POLICY "Users can insert own articles" ON public.articles FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own articles' AND tablename = 'articles') THEN
        CREATE POLICY "Users can update own articles" ON public.articles FOR UPDATE USING (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own articles' AND tablename = 'articles') THEN
        CREATE POLICY "Users can delete own articles" ON public.articles FOR DELETE USING (auth.uid() = user_id);
    END IF;

    -- article_charts policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own charts' AND tablename = 'article_charts') THEN
        CREATE POLICY "Users can view own charts" ON public.article_charts FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own charts' AND tablename = 'article_charts') THEN
        CREATE POLICY "Users can insert own charts" ON public.article_charts FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own charts' AND tablename = 'article_charts') THEN
        CREATE POLICY "Users can update own charts" ON public.article_charts FOR UPDATE USING (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own charts' AND tablename = 'article_charts') THEN
        CREATE POLICY "Users can delete own charts" ON public.article_charts FOR DELETE USING (auth.uid() = user_id);
    END IF;

    -- article_chat policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own chat' AND tablename = 'article_chat') THEN
        CREATE POLICY "Users can view own chat" ON public.article_chat FOR SELECT USING (auth.uid() = user_id OR is_admin(auth.uid()));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own chat' AND tablename = 'article_chat') THEN
        CREATE POLICY "Users can insert own chat" ON public.article_chat FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own chat' AND tablename = 'article_chat') THEN
        CREATE POLICY "Users can update own chat" ON public.article_chat FOR UPDATE USING (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own chat' AND tablename = 'article_chat') THEN
        CREATE POLICY "Users can delete own chat" ON public.article_chat FOR DELETE USING (auth.uid() = user_id);
    END IF;

    -- files policies (DEPRECATED)
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own files' AND tablename = 'files') THEN
        CREATE POLICY "Users can view own files" ON public.files FOR SELECT USING (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own files' AND tablename = 'files') THEN
        CREATE POLICY "Users can insert own files" ON public.files FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own files' AND tablename = 'files') THEN
