...
export const load: PageServerLoad = async ({ locals: { safeGetSession, supabase } }) => {
	...
	const log = createLogger('TodoPageLoad', user?.id);
	const monitor = createQueryMonitor(log, 100); // 100ms threshold

	// Fetch last N Superjournal entries for TODO mode, newest first (paginated)
	...
	// Fetch starred journal entries to get their superjournal_ids (todo mode only)
