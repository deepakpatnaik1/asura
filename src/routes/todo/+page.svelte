<script lang="ts">
	...
	onMount(() => {
		...
		if (typeof window !== 'undefined') {
			localStorage.setItem('asura_app_mode', 'todo');
		}
	...
	async function loadMoreMessages() {
		...
		isLoadingMore = true;
		try {
			const response = await fetch(`/api/superjournal?offset=${currentOffset}&limit=50&mode=todo`);
			if (response.ok) {
	...
	$effect(() => {
		...
		for (const p of personas) {
			if (p.mode === 'todo' && normalized.startsWith(p.name)) {
	...
	async function selectPersona(persona: string) {
		// Check if persona is available in todo mode
		const personas = data.personas || [];
		const personaData = personas.find((p) => p.name === persona);
		if (!personaData || personaData.mode !== 'todo') return;
	...
	async function handleSend() {
		...
		await sendMessage(message, selectedPersona, chartId, chartSource, 'todo');
