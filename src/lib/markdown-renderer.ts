...
function processInlineFormatting(text: string, accent: string, accentBg: string): string {
	...
	const codeStyle = `background: ${CODE_BLOCK_BG}; padding: 2px 6px; border-radius: 3px; font-family: 'SF Mono', Monaco, monospace; font-size: 0.9em; word-break: break-word;`;
...
export async function renderMarkdown(
	...
	processed = processed.replace(codeBlockRegex, (match, lang, code) => {
		...
		const escapedCode = processedCode
			...
			.replace(/</g, '&lt;')
			.replace(/>/g, '&gt;')
			// Em dash → en dash with spaces (British style)
			.replace(/\s*—\s*/g, ' – ')
	...
	processed = processed.replace(/https?:\/\/[^\s<>"]+/gi, (url) => {
		...
		globalUrls.push(`<a href="${url}" target="_blank" rel="noopener" style="display: inline-block; padding: 2px 8px; border-radius: 10px; background: ${ACCENT_BG}; color: ${ACCENT}; font-size: 0.85em; text-decoration: none;">click here</a>`);
		return placeholder;
	});
	...
	processed = processed.replace(blockQuoteRegex, (match) => {
		...
		for (const line of lines) {
			...
			if (content) {
				...
				const quoteCodeStyle = `background: ${CODE_BLOCK_BG}; padding: 2px 6px; border-radius: 3px; font-family: 'SF Mono', Monaco, monospace; font-size: 0.9em; word-break: break-word;`;
	...
	// Hyphen as dash (British style: en dash with spaces)
	// Convert when adjacent to contractions (word's, they're, etc.) - preserves compound words
	processed = processed.replace(/(\w)-(\w+'\w+)/g, '$1 – $2');  // word-that's → word – that's
	processed = processed.replace(/(\w+'\w+)-(\w)/g, '$1 – $2');  // that's-word → that's – word
	...
	for (const line of lines) {
		...
		// Markdown headers (# through ######) → accent colored, sized by level
		// With margin annotation (h1-h6) that's visible but not selectable
		...
		if (headerMatch) {
			...
			const headerCodeStyle = `background: ${CODE_BLOCK_BG}; padding: 2px 6px; border-radius: 3px; font-family: 'SF Mono', Monaco, monospace; font-size: 0.9em; word-break: break-word;`;
			...
			// Size decreases with level: h1=1.45em down to h6=0.95em
		...
		if (line.match(/^-{3,}$/) || line.match(/^\*\s*\*\s*\*$/) || line.match(/^\*{3,}$/)) {
			...
			results.push(`<div class="flourish-divider" style="display: flex; justify-content: center; margin: 0.8em 0;">${flourishSvg}</div>`);
			continue;
		}
		...
		if (bulletMatch) {
			...
			rest = rest.replace(/r\/\w+\s*\(https?:\/\/(?:www\.)?reddit\.com\/r\/(\w+)\/?(?:\?[^)]*)?\)/gi, (match, sub) => {
				...
				return `<a href="${url}" target="_blank" rel="noopener" style="display: inline-block; padding: 2px 8px; border-radius: 10px; background: ${ACCENT_BG}; color: ${ACCENT}; font-size: 0.85em; text-decoration: none;">click here</a>`;
			});
			rest = rest.replace(/\[([^\]]+)\]\(([^)]+)\)/g, `<a href="$2" target="_blank" rel="noopener" style="display: inline-block; padding: 2px 8px; border-radius: 10px; background: ${ACCENT_BG}; color: ${ACCENT}; font-size: 0.85em; text-decoration: none;">$1</a>`);
			// Auto-link bare URLs (not already in markdown link) → pill badge
			rest = rest.replace(/(?<!href="|]\()https?:\/\/[^\s<>"()]+/g, (url) => {
		...
		escaped = escaped.replace(/https?:\/\/[^\s<>"]+/g, (url) => {
			...
			urlPlaceholders.push(`<a href="${url}" target="_blank" rel="noopener" style="display: inline-block; padding: 2px 8px; border-radius: 10px; background: ${ACCENT_BG}; color: ${ACCENT}; font-size: 0.85em; text-decoration: none;">click here</a>`);
			return placeholder;
		});
		...
		const lineCodeStyle = `background: ${CODE_BLOCK_BG}; padding: 2px 6px; border-radius: 3px; font-family: 'SF Mono', Monaco, monospace; font-size: 0.9em; word-break: break-word;`;
		...
		// Reddit subreddit pattern: "r/Name (https://reddit.com/r/...)" → single badge
		escaped = escaped.replace(/r\/\w+\s*\(https?:\/\/(?:www\.)?reddit\.com\/r\/(\w+)\/?(?:\?[^)]*)?\)/gi, (match, sub) => {
			const url = `https://www.reddit.com/r/${sub}/`;
			return `<a href="${url}" target="_blank" rel="noopener" style="display: inline-block; padding: 2px 8px; border-radius: 10px; background: ${ACCENT_BG}; color: ${ACCENT}; font-size: 0.85em; text-decoration: none;">click here</a>`;
		});
