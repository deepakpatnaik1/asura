<script lang="ts">
	/**
	 * CanvasLibrary - Dropdown for managing Eva's character design canvases
	 *
	 * Similar to BoardLibrary:
	 * - Checkbox toggle for context injection (multi-select)
	 * - Canvases can be enabled/disabled independently
	 * - Click title to view/edit canvas
	 */
	...
	interface Canvas {
		...
		title: string;
		state?: {
			render: Array<{ id: string; type: string; x?: number; y?: number; text?: string; fill?: string; width?: number; height?: number; src?: string }>;
	...
	let { canvases, selectedIds, isDeleting = false, onToggle, onOpen, onDelete, onClear }: Props = $props();
...
<div class="canvas-library-dropdown">
	{#if canvases.length === 0}
		<div class="dropdown-empty">No canvases yet</div>
	{:else}
		...
		{#each canvases as canvas (canvas.id)}
			...
			>
				...
				<button
					class="toggle-btn"
					class:active={isSelected}
					onclick={(e) => handleToggleClick(canvas.id, e)}
				...
				<div class="canvas-info">
					<button class="title-btn" onclick={(e) => handleOpenClick(canvas.id, e)} title="Open canvas">
						<span class="canvas-title">{canvas.title}</span>
					</button>
					<span class="canvas-date">{formatDate(canvas.updated_at)}</span>
				</div>
				{#if onDelete}
					<button
						class="delete-btn"
