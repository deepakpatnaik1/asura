# Implementation Plan: BUG-016 and BUG-017 Fixes
...
## Step 1: Create New Function - `createFilePending()`
...
2. Sets progress to 0 (not 25) since extraction doesn't count as complete yet
...
**Rationale**: Client will see "Pending 0%" immediately, then background processing will update it to 25%, 75%, 90%, 100%.

---

## Step 2: Create New Function - `processFileBackground()`
...
## Step 3: Refactor Existing `processFile()`
...
try {
  ...
  if (error instanceof FileProcessorError) {
    // Known error types: VALIDATION_ERROR, EXTRACTION_ERROR, DUPLICATE_FILE, DATABASE_ERROR
    return json(
      {
        error: {
...
**Errors from `createFilePending()`** (should return HTTP error):
- `VALIDATION_ERROR`: 400 Bad Request
- `EXTRACTION_ERROR`: 400 Bad Request
- `DUPLICATE_FILE`: 400 Bad Request
...
## Step 5: Verify SSE and Client Matching
...
**Changes**:
1. Split `processFile()` into `createFilePending()` + `processFileBackground()`
2. Modify upload endpoint to await `createFilePending()` and return real ID
3. Keep `processFile()` as wrapper for backward compatibility
...
### Phase 2: BUG-017 - Create New Functions (30 minutes)
...
### Phase 3: BUG-017 - Modify Upload Endpoint (15 minutes)
...
2. Run `npm run dev`
3. Execute all test cases from testing plan
...
- [ ] Error handling works (invalid files rejected)
...
2. `/Users/d.patnaik/code/asura/src/lib/file-processor.ts` (add 2 functions, refactor 1)
...
1. Is the 1-second delay for upload response acceptable?
2. Should we keep `processFile()` as wrapper or delete it?
3. Should we add logging for ID matching (debugging)?
4. Should we add automated tests before or after implementation?
5. Any concerns about the split architecture?

---

**Plan Status**: Ready for Review
**Target Score**: ≥8/10
**Next Step**: Reviewer feedback → Implementation → Testing → Documentation
